import { pseudoRandom } from "../utils/random.js";export function generateSessionParameters(seed,opts={}){const rnd=pseudoRandom(seed || Date.now());const themes=['Поток сознания','Хроногнозия','Семантические сети','Метакогниция','Аффективный ландшафт','Перцептуальные сдвиги'];const modifiers=[ 'Инверсия восприятия','Туннельное зрение','Хроматическая адаптация','Рабочая память','Когнитивные искажения','Эмоциональные барьеры','Локальные временные поля' ];const theme=opts.theme || themes[Math.floor(rnd()*themes.length)];const difficulty=opts.difficulty ||(1+Math.floor(rnd()*5));const modCount=1+Math.floor(rnd()*3);const mods=[];const used=new Set();while(mods.length<modCount){const m=modifiers[Math.floor(rnd()*modifiers.length)];if(!used.has(m)){used.add(m);mods.push(m);}}const level={gravity:0.5+rnd()*1.2*(1+(difficulty-1)*0.15),gap:Math.round(80+rnd()*160/difficulty),speed:2+rnd()*3*(1+(difficulty-1)*0.2),colorSeed:Math.floor(rnd()*360)};return{seed:seed||Date.now(),theme,difficulty,modifiers:mods,level};}export const CHAOTIC_MODIFIERS=['Инверсия управления','Гравитационные всплески','Телепорт-хаос','Дрейф платформ','Туннельное зрение'];
