...
import audioManager from '../systems/audio_manager.js';import{generateSessionParameters}from '../systems/proc_gen.js';const RULE_BANK=[ "Инверсия восприятия — управление инвертируется при моргании.","Туннельное зрение — фокус сужается при стрессе.","Рабочая память:удерживай комбинации в уме для прогресса.","Когнитивные искажения:платформы меняют свойства при наблюдении.","Эмоциональные барьеры:проходимы только в нужном настроении.","Перцептивные иллюзии:объекты обманывают восприятие.","Локальные временные поля:управление временем внутри зон.","Семантические связи:мосты между концептуальными узлами.","Рефлексивные поверхности:показывают скрытые слои реальности.","Аффективные резонансы:усиливают определённые эмоции.","Мнемонические якоря:точки сохранения состояния игрока.","Динамическая система сложности:подстраивается под поведение игрока.","Адаптивный саундскейп:меняется в реальном времени.","Процедурный конструктор уровней с тематическими зонами.","Полный психо-синтез:комбинация 3+режимов." ];const GAMES=[{id:'flappy',name:'Flappy Bird',demo:'/src/modes/demo_games/flappy_demo.js'},{id:'doodle',name:'Doodle Jump',demo:'/src/modes/demo_games/doodle_demo.js'},{id:'snake',name:'Snake',demo:'/src/modes/demo_games/snake_demo.js'},{id:'dino',name:'Dino Run',demo:'/src/modes/demo_games/dino_demo.js'},{id:'breakout',name:'Breakout',demo:'/src/modes/demo_games/breakout_demo.js'},{id:'2048',name:'2048',demo:'/src/modes/demo_games/2048_demo.js'}];function pickRandom(arr,n){const copy=arr.slice();const out=[];n=Math.max(1,Math.min(n,arr.length));for(let i=0;i<n;i++){const idx=Math.floor(Math.random()*copy.length);out.push(copy.splice(idx,1)[0]);}return out;}function generateSession(){const seed=Math.floor(Math.random()*0xffffffff);const params=generateSessionParameters(seed);const rulesCount=3+Math.floor(Math.random()*4);const rules=pickRandom(RULE_BANK,rulesCount);const chosenGame=GAMES[Math.floor(Math.random()*GAMES.length)];const header=`Дата:${new Date().toLocaleString('ru-RU')}\nSeed:${params.seed}\nТема:${params.theme}\nСложность:${params.difficulty}\nИгра:${chosenGame.name}\nМодификаторы:${params.modifiers.join(',')}\n\n`;const body=rules.map((r,i)=>`${i+1}. ${r}`).join('\n');const footer="\n\nВот такое";return{text:header+body+footer,game:chosenGame,params};}const tg=(window.Telegram && window.Telegram.WebApp)? window.Telegram.WebApp:null;if(tg){try{tg.expand();tg.ready();}catch(e){}}const el={menu:document.getElementById('menu'),generateBtn:document.getElementById('generate-btn'),session:document.getElementById('session'),sessionText:document.getElementById('session-text'),regen:document.getElementById('regen-btn'),play:document.getElementById('play-btn'),download:document.getElementById('download-btn'),send:document.getElementById('send-btn'),back:document.getElementById('back-btn'),launcher:document.getElementById('game-launcher'),launchMsg:document.getElementById('launch-msg'),gameRoot:document.getElementById('game-root'),stopBtn:document.getElementById('stop-game')};let current=null;try{audioManager.init();}catch(e){}let runningInstance=null;function showSession(obj){
